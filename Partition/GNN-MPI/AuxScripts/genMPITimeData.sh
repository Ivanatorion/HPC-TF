...
cp GNN/model_weights_high.dat GNN/model_weights.dat
...
do
   ...
   p="GNN/Partitions/G$j.2"
   for (( k=0; k<${#cores[@]}; k++ ))
   do
       mpirun -np ${cores[k]} gnnmpi $f $p 1 > outtimedata.txt
...
do
    ...
    for (( k=0; k<${#cores[@]}; k++ ))
    do
        mpirun -np ${cores[k]} gnnmpi $f $p 0 > outtimedata.txt
        AuxScripts/processRastro.sh
        data=$(cat outtimedata.txt)
        data=($data)
        base=${data[5]}

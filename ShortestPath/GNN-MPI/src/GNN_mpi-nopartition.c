...
void GNN_FixGraphHSDim(GNN* gnn, Graph *graph){
    ...
    for(i = 0; i < graph->nEdges; i++){
        memcpy(hsAux, graph->edges[i].hiddenState, elemsToCopy * sizeof(double));
        free(graph->edges[i].hiddenState);
        graph->edges[i].hiddenState = (double *) malloc(sizeof(double) * gnn->hiddenStateDim);
        memset(graph->edges[i].hiddenState, 0, sizeof(double) * gnn->hiddenStateDim);
        memcpy(graph->edges[i].hiddenState, hsAux, sizeof(double) * elemsToCopy);
    }

    free(hsAux);
}

void Partition(Graph *graph, int *graphPartition, int nParts){
    char buffer[256];
    sprintf(buffer, "Partitions/Graph.part.%d", nParts);
...
void GNN_Apply(GNN* gnn, Graph *graph, double *outputPerNode){
    ...
    int *graphPartition = (int *) malloc(sizeof(int) * graph->nNodes);
    if(size == 1)
        memset(graphPartition, 0, sizeof(int) * graph->nNodes);
    else
        Partition(graph, graphPartition, size);
    ...
    double *msgInput = (double *) malloc(HIDDEN_STATE_BYTES + HIDDEN_STATE_BYTES + sizeof(double)); //[source, destination, $weight]
            ...
            if(graphPartition[j] == rank){
                for(k = 0; k < graph->nodes[j].nInEdges; k++){
                    ...
                    if(sourceOwner != rank){
                        ...
                        memcpy(msgInput + 2 * graph->hiddenStateDim, &graph->edges[graph->nodes[j].inEdgesIndexes[k]].hiddenState[0], sizeof(double));

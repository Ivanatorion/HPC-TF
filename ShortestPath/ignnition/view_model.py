...
def main():
    ...
    f = open("model_description.yaml")
    md_data = yaml.safe_load(f)
    f.close()

    num_iterations = md_data["message_passing"]["num_iterations"]
    agg_type = md_data["message_passing"]["stages"][0]["stage_message_passings"][0]["aggregation"][0]["type"]
    hs_dim = md_data["entities"][0]["state_dimension"]
    ...
    with open("model_weights.dat", "w") as out_file:
        ...
        out_file.write(str(int(num_iterations)) + "\n")
        out_file.write(str(agg_type) + "\n")
        out_file.write(str(hs_dim) + "\n")
